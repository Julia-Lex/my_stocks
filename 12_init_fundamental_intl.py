...
美股专属三条裁定(H2 审查发现,brief 之外由控制者拍板,见本文件注释(三期直写,无独立报告)):
  1. ann_date 护栏:东财"累计季报"/"年报"接口的 NOTICE_DATE 对老报告期存在系统性"被最新一次
     同类披露覆盖"的缺陷(实测 AAPL 历史行大量出现 ~390-800 天的假滞后,仅最新一期 NOTICE_DATE
     可信)。美股 10-K/10-Q 法定披露滞后 ≤90 天,原 400 天阈值形同虚设 —— 最终审查对已入库
     ann_date 实测:78% 的"幸存"行(即通过了旧 400 天阈值)恰好落在 380-401 天这个窄带,
     与法定披露窗口(≤90 天)完全不重叠,确证是"被下次同类披露覆盖"的假值而非真实滞后披露。
     阈值收紧为 200 天(见下方 _ANN_GUARD_DAYS 注释),回填前丢弃 (ann_date - report_date)
     > 200 天的行,丢弃计数打 log(方向仍偏保守,不保证拦住全部假值,见下方注释疑虑)。
...
def _clean_num(v, col: str):
    ...
    pd.isna(v) 覆盖 pd.NA/np.nan/None 全部标量空值(而非只判 isinstance float),
    对标量安全(不会在 Series/array 上触发 ambiguous truth value)——原判断遗漏
    pd.NA(非 float 类型)时会漏判直接进 float(v) 抛异常,2026-07-11 最终审查发现。
    """
...
# ===========================================================================
# 探测结论(2026-07-10,AAPL "年报"+"累计季报" 全历史 78 行实测;2026-07-11 最终审查在
# 全量入库数据上复核,见本文件注释):东财 NOTICE_DATE 对同一 REPORT_TYPE 的老报告期存在
# 系统性"被下一次同类披露覆盖"的缺陷 —— 每个 report_date 只有"当前最新一期"的 NOTICE_DATE
# 是真实近端披露日(约 30-40 天滞后),更老的报告期的 NOTICE_DATE 全部指向"下一次同类报表
# 披露"的日期(实测滞后集中在 ~390-410 天(累计季报)与 ~760-810 天(年报),个别更极端到
# 850+ 天)。旧阈值 400 天仅拦住年报族异常行(除刚好 398 天的次新一期)和约半数累计季报族
# 异常行,对 380-401 天这个窄带完全放行 —— 而最终审查对已入库 us_fin_indicator 实测:
# ann_date 非空的行里 78% 恰好落在这个窄带,与美股 10-K/10-Q 法定披露滞后上限(≤90 天)
# 相去甚远,确证是假滞后而非真实晚披露。收紧阈值至 200 天:高于法定披露上限两倍以上留出
# 余量,同时落在假值最密集的 380-401 天带之外,能拦住该带全部行(仍不保证拦住 200-380
# 天之间可能存在的更隐蔽假值,方向保守优先,见 README 三期"已知限制")。
_ANN_GUARD_DAYS = 200
