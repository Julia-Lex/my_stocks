...
## 数据源 · 2026-07-10 晚间修订:富途升级为主源

> 用户提供富途账号 + FutuOpenD 网关后实测 `get_financials_statements`(futu-api 10.08.6808):港股 00700 回溯 **2001**(115 期)、美股 AAPL 回溯 **1979**(216 期)、A 股也通(将来可反哺);statement_type 整数枚举(1利润/2资产负债/3现金流/4关键指标);响应含 field_id 稳定编码、currency_code、会计准则;限频 30 次/30 秒(官方明确)。**但同样无公告日**。

**修订后分工**:
- **报表层(JSONB)主源 = 富途** types 1/2/3,item_list pivot 成 `{display_name: data}` 入 JSONB(field_id 一并存,键 `_fid_{id}` 或平行数组,实施定);东财长表实现保留为备源(`ASTOCK_INTL_FUND_SOURCE=em` 开关,futu 默认)。
- **指标层主源 = 富途 type 4(关键指标)** + 东财指标接口**专职补 ann_date**(美股 NOTICE_DATE 已确认;港股按下文三级探测)。指标宽表列映射自富途关键指标 display_name。
- 吞吐:限频 1 请求/秒 → 港股全量 ~2,809 × 4 类 × ~2 页 ≈ 2.2 万请求 ≈ **6 小时过夜跑**;美股 ~1.2 小时。断点续传/熔断照旧。
- **运维新依赖**:FutuOpenD 网关常驻(增量 cron 同样依赖);网关不可达时脚本明确报错退出(不静默降级),README 记录网关启动方式。

## 原东财实探(降为备源依据,2026-07-10 日间)
